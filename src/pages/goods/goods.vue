...
// 新增购物车
const onPostCart = async (ev: SkuPopupEvent) => {
  ...
  <template v-else>
    <scroll-view scroll-y class="viewport">
      ...
      <view class="goods">
        ...
        <!-- 操作面板 -->
        <view class="action">
          <view @tap="onOpenSkuPopup(SkuMode.Both)" class="item arrow">
      ...
      <uni-popup ref="popup" type="bottom" border-radius="10px 10px 0 0">
        <AddressPanel v-if="PopupName === 'address'" @closePopup="popup?.close()" />
        <ServicePanel v-if="PopupName === 'service'" @closePopup="popup?.close()" />
      </uni-popup>
    ...
    <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
      ...
      <view class="buttons">
        <view @tap="onOpenSkuPopup(SkuMode.Cart)" class="addcart"> 加入购物车 </view>
        <view @tap="onOpenSkuPopup(SkuMode.Buy)" class="buynow"> 立即购买 </view>
      </view>
    </view></template
  >
</template>
